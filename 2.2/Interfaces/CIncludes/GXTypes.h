...
enum {
	gxLeftJustify				= 0,
	gxCenterJustify				= ((Fract)0x40000000L) / 2,
	gxRightJustify				= ((Fract)0x40000000L),
	gxFillJustify				= -1
};
...
struct gxTextFace {
	...
	struct gxFaceLayer				faceLayer[1];				/* zero or more face layers describing the face */
};
...
typedef long (*gxSpoolProcPtr)(gxSpoolCommand command, gxSpoolBlock *block);
...
enum {
	uppgxSpoolProcInfo = kCStackBased
		 ...
		 | STACK_ROUTINE_PARAMETER(2, SIZE_CODE(sizeof(gxSpoolBlock*)))
};

#if GENERATINGCFM
#define NewgxSpoolProc(userRoutine)		\
		(gxSpoolUPP) NewRoutineDescriptor((ProcPtr)(userRoutine), uppgxSpoolProcInfo, GetCurrentArchitecture())
#else
#define NewgxSpoolProc(userRoutine)		\
...
enum {
	...
	gxMaxRunLevel				= 15,
	gxFlushLeft					= 0,
	gxFlushCenter				= ((Fract)0x40000000L) / 2,
	gxFlushRight				= ((Fract)0x40000000L),
	gxNoJustification			= 0,
	gxFullJustification			= ((Fract)0x40000000L)
};
...
struct gxPriorityJustificationOverride {
	struct gxWidthDeltaRecord		deltas[gxNumberOfJustificationPriorities]; /* overrides for each of the priorities */
};
...
enum {
	gxResetCrossStreamFactor	= ((long)0x80000000L)
};
...
/* old header = graphics stream types */
...
#define gxSynonymTags
/* old header = synonym tags */
...
typedef enum gxCubicSynonym gxCubicSynonym;
enum {

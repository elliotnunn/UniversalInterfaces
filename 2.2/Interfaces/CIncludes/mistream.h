/*
	...
	Change History (most recent first):
				 ...
				 2/12/96	pfl		xxx put comment here xxx

	To Do:
		• Fix up the pragma options.
		§
*/

/** Copyright (c) 1989-1994  Mentat Inc.
 ** stream.h 4.7, last change 10 Apr 1994
 **/
...
typedef	int	(* _CDECL putp_t)(queue_t *, struct msgb *);
typedef	int	(* _CDECL srvp_t)(queue_t *);
typedef int (* _CDECL admin_t)(void);
typedef void (* _CDECL bufcall_t)(long);
typedef int	(* _CDECL openp_t)(queue_t*, dev_t*, int, int, cred_t*);
typedef int (* _CDECL closep_t)(queue_t*, int, cred_t*);
typedef int	(* _CDECL openOld_t)(queue_t*, dev_t, int, int);
typedef int (* _CDECL closeOld_t)(queue_t*);
...
{
	void	(* _CDECL free_func)(char* arg); 	/* Routine to call to free buffer */
...
queue_t*	allocq(void);
int			adjmsg(mblk_t*, int len_param);
queue_t*	backq(queue_t*);

int		bcanput(queue_t*, uchar_p pri);
int		bcanputnext(queue_t*, uchar_p pri);
int		bufcall(size_t memNeeded, int pri, bufcall_t,	// %%%
				long context);
int		canput(queue_t*);
...
mblk_t*	esballoc(unsigned char* base, size_t size, int pri, frtn_t*);
void	flushband(queue_t*, uchar_p pri, int flag);
void	flushq(queue_t*, int flag);
void	freeb(mblk_t*);
void	freemsg(mblk_t*);
int		freeq(queue_t*);
int		freezestr(queue_t*);
admin_t	getadmin(ushort_p mid);
unsigned short	getmid(char * name);
mblk_t*	getq(queue_t*);
int		insq(queue_t*, mblk_t* emp, mblk_t* nmp);
void	linkb(mblk_t* mp1, mblk_t* mp2);
int		msgdsize(const mblk_t*);
mblk_t*	msgpullup(mblk_t*, int len);
int		pullupmsg(mblk_t*, int len);
int		putbq(queue_t*, mblk_t*);
int		putctl(queue_t*, int type);
int		putnextctl(queue_t*, int type);
int		putctl1(queue_t*, int type, int c);
int		putnextctl1(queue_t*, int type, int c);
int		putctl2(queue_t*, int type, int c1, int c2);
#ifndef puthere
int		puthere(queue_t*, mblk_t*);
#endif
int		putnext(queue_t*,  mblk_t*);
int		putq(queue_t*, mblk_t*);
int		putq_owned(queue_t*, mblk_t*);									// this line deleted for release disk
void	qenable(queue_t*);
void	qprocson(queue_t*);
void	qprocsoff(queue_t*);
int		qreply(queue_t*, mblk_t*);
int		qsize(queue_t*);
mblk_t*	rmvb(mblk_t * mp, mblk_t * bp);
void	rmvq(queue_t*, mblk_t*);
/* prototype for strlog in strlog.h */
int		strqget(queue_t*, qfields_t what, uchar_p pri, long* valp);
int		strqset(queue_t*, qfields_t what, uchar_p pri, long val);
int		testb(int size, int pri);
void	unbufcall(int id);
void	unfreezestr(queue_t * q, int oldpri);

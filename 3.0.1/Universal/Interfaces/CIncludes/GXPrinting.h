...
#define Send_GXJobIdle() SendMessage(0x00000002)
...
#define Send_GXJobStatus(pStatus) \
		  SendMessage(0x00000003, pStatus)
...
#define Send_GXPrintingEvent(evtRecord, filterEvent) \
		  SendMessage(0x00000004, evtRecord, filterEvent)
...
#define Send_GXJobDefaultFormatDialog(dlgResult) \
		  SendMessage(0x00000005, dlgResult)
...
#define Send_GXFormatDialog(theFormat, title, dlgResult) \
		  SendMessage(0x00000006, theFormat, title, dlgResult)
...
#define Send_GXJobPrintDialog(dlgResult) \
		  SendMessage(0x00000007, dlgResult)
...
#define Send_GXFilterPanelEvent(pHitInfo, returnImmed) \
		  SendMessage(0x00000008, pHitInfo, returnImmed)
...
#define Send_GXHandlePanelEvent(pHitInfo, panelResponse) \
		  SendMessage(0x00000009, pHitInfo, panelResponse)
...
#define Send_GXParsePageRange(fromString, toString, result) \
		  SendMessage(0x0000000A, fromString, toString, result)
...
#define Send_GXDefaultJob() SendMessage(0x0000000B)
...
#define Send_GXDefaultFormat(theFormat) \
		  SendMessage(0x0000000C, theFormat)
...
#define Send_GXDefaultPaperType(thePaperType) \
		  SendMessage(0x0000000D, thePaperType)
...
#define Send_GXDefaultPrinter(thePrinter) \
		  SendMessage(0x0000000E, thePrinter)
...
#define Send_GXCreateSpoolFile(pFileSpec, createOptions, theSpoolFile) \
		  SendMessage(0x0000000F, pFileSpec, createOptions, \
		  				  theSpoolFile)
...
#define Send_GXSpoolPage(theSpoolFile, theFormat, thePage) \
		  SendMessage(0x00000010, theSpoolFile, theFormat, thePage)
...
#define Send_GXSpoolData(theSpoolFile, data, length) \
		  SendMessage(0x00000011, theSpoolFile, data, length)
...
#define Send_GXSpoolResource(theSpoolFile, theResource, theType, id) \
		  SendMessage(0x00000012, theSpoolFile, theResource, \
		  				  theType, id)
...
#define Send_GXCompleteSpoolFile(theSpoolFile) \
		  SendMessage(0x00000013, theSpoolFile)
...
#define Send_GXCountPages(theSpoolFile, numPages) \
		  SendMessage(0x00000014, theSpoolFile, numPages)
...
#define Send_GXDespoolPage(theSpoolFile, numPages, theFormat, thePage, formatChanged) \
		  SendMessage(0x00000015, theSpoolFile, numPages, \
...
#define Send_GXDespoolData(theSpoolFile, data, length) \
		  SendMessage(0x00000016, theSpoolFile, data, length)
...
#define Send_GXDespoolResource(theSpoolFile, theType, id, theResource) \
		  SendMessage(0x00000017, theSpoolFile, theType, \
		  				  id, theResource)
...
#define Send_GXCloseSpoolFile(theSpoolFile, closeOptions) \
		  SendMessage(0x00000018, theSpoolFile, closeOptions)
...
#define Send_GXStartJob(docName, pageCount) \
		  SendMessage(0x00000019, docName, pageCount)
...
#define Send_GXFinishJob() SendMessage(0x0000001A)
...
#define Send_GXStartPage(theFormat, numViewPorts, viewPortList) \
		  SendMessage(0x0000001B, theFormat, numViewPorts, viewPortList)
...
#define Send_GXFinishPage() SendMessage(0x0000001C)
...
#define Send_GXPrintPage(theFormat, thePage) \
		  SendMessage(0x0000001D, theFormat, thePage)
...
#define Send_GXSetupImageData(imageData) \
		  SendMessage(0x0000001E, imageData)
...
#define Send_GXImageJob(theSpoolFile, closeOptions) \
		  SendMessage(0x0000001F, theSpoolFile, closeOptions)
...
#define Send_GXImageDocument(theSpoolFile, imageData) \
		  SendMessage(0x00000020, theSpoolFile, imageData)
...
#define Send_GXImagePage(theSpoolFile, pageNumber, theFormat, imageData) \
		  SendMessage(0x00000021, theSpoolFile, pageNumber, theFormat, imageData)
...
#define Send_GXRenderPage(theFormat, thePage, pageInfo, imageData) \
		  SendMessage(0x00000022, theFormat, thePage, pageInfo, imageData)
...
#define Send_GXCreateImageFile(pFileSpec, imageFileOptions, theImageFile) \
		  SendMessage(0x00000023, pFileSpec, imageFileOptions, theImageFile)
...
#define Send_GXOpenConnection() SendMessage(0x00000024)
...
#define Send_GXCloseConnection() SendMessage(0x00000025)
...
#define Send_GXStartSendPage(theFormat) SendMessage(0x00000026, theFormat)
...
#define Send_GXFinishSendPage() SendMessage(0x00000027)
...
#define Send_GXWriteData(data, length) SendMessage(0x00000028, data, length)
...
#define Send_GXBufferData(data, length, bufferOptions) \
		  SendMessage(0x00000029, data, length, bufferOptions)
...
#define Send_GXDumpBuffer(theBuffer) SendMessage(0x0000002A, theBuffer)
...
#define Send_GXFreeBuffer(theBuffer) SendMessage(0x0000002B, theBuffer)
...
#define Send_GXCheckStatus(data, length, statusType, owner) \
		  SendMessage(0x0000002C, data, length, statusType, owner)
...
#define Send_GXGetDeviceStatus(cmdData, cmdSize, responseData, responseSize, termination) \
		  SendMessage(0x0000002D, cmdData, cmdSize, responseData, responseSize, termination)
...
#define Send_GXFetchTaggedData(theType, id, dataHdl, owner) \
		  SendMessage(0x0000002E, theType, id, dataHdl, owner)
...
#define	Send_GXGetDTPMenuList(menuHdl) \
			SendMessage(0x0000002F, menuHdl)
...
#define	Send_GXHandleAlertFilter(theJob, pStatusRec, pDialog, theEvent, itemHit, returnImmed) \
			SendMessage(0x00000031, theJob, pStatusRec, pDialog, theEvent, itemHit, returnImmed)
...
#define Send_GXJobFormatModeQuery(theQuery, srcData, dstData) \
		  SendMessage(0x00000032, theQuery, srcData, dstData)
...
#define Send_GXWriteStatusToDTPWindow(pStatusRec, pDisplay) \
		  SendMessage(0x00000033, pStatusRec, pDisplay)
...
#define Send_GXInitializeStatusAlert(pStatusRec, pDialog) \
		  SendMessage(0x00000034, pStatusRec, pDialog)
...
#define Send_GXHandleAlertStatus(pStatusRec) \
		  SendMessage(0x00000035, pStatusRec)
...
#define Send_GXHandleAlertEvent(pStatusRec, pDialog, theEvent, response) \
		  SendMessage(0x00000036, pStatusRec, pDialog, theEvent, response)
...
#define Send_GXCleanupStartJob() ((void) SendMessage(0x00000037))
...
#define Send_GXCleanupStartPage() ((void) SendMessage(0x00000038))
...
#define Send_GXCleanupOpenConnection() ((void) SendMessage(0x00000039))
...
#define Send_GXCleanupStartSendPage()  ((void) SendMessage(0x0000003A))
...
#define Send_GXDefaultDesktopPrinter(dtpName) SendMessage(0x0000003B, dtpName)
...
#define Send_GXCaptureOutputDevice(capture) SendMessage(0x0000003C, capture)
...
#define Send_GXOpenConnectionRetry(theType, aVoid, retryopenPtr, anErr) \
		  SendMessage(0x0000003D, theType, aVoid, retryopenPtr, anErr)
...
#define Send_GXExamineSpoolFile(theSpoolFile) SendMessage(0x0000003E, theSpoolFile)
...
#define Send_GXFinishSendPlane() SendMessage(0x0000003F)
...
#define Send_GXDoesPaperFit(whichTray, paper, fits) \
		  SendMessage(0x00000040, whichTray, paper, fits)
...
#define Send_GXChooserMessage(message, caller, objName, zoneName, theList, p2) \
		  SendMessage(0x00000041, message, caller, objName, zoneName, theList, p2)
...
#define Send_GXFindPrinterProfile(thePrinter, searchData, index, returnedProfile, numProfiles) \
		  SendMessage(0x00000042, thePrinter, searchData, index, returnedProfile, numProfiles)
...
#define Send_GXFindFormatProfile(theFormat, searchData, index, returnedProfile, numProfiles) \
		  SendMessage(0x00000043, theFormat, searchData, index, returnedProfile, numProfiles)
...
#define Send_GXSetPrinterProfile(thePrinter, oldProfile, newProfile) \
		  SendMessage(0x00000044, thePrinter, oldProfile, newProfile)
...
#define Send_GXSetFormatProfile(theFormat, oldProfile, newProfile) \
		  SendMessage(0x00000045, theFormat, oldProfile, newProfile)
...
#define Send_GXHandleAltDestination(userCancels) SendMessage(0x00000046, userCancels)
...
#define Send_GXSetupPageImageData(theFormat, thePage, imageData) \
		  SendMessage(0x00000047, theFormat, thePage, imageData)

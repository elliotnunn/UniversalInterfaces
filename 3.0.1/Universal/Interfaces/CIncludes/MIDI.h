...
struct MIDIPortParams {
	...
	MIDIReadHookProcPtr 			readHook;					/*routine to call when input data is valid*/
...
struct MDVRInCtlRec {
	...
	MDVRReadProcPtr 				readProc;					/*proc to call with intput characters*/
	MDVRCommProcPtr 				commProc;					/*proc to call for handshaking*/
...
struct MDVROutCtlRec {
	...
	MDVRTimeCodeProcPtr 			timeCodeProc;				/*proc called on time code fmt change*/
	MDVRCommProcPtr 				commProc;					/*proc called for handshaking*/
...
MIDIWakeUp						(short 					refnum,
								 long 					time,
								 long 					period,
								 MIDITimeProcPtr 		timeProc)							FOURWORDINLINE(0x203C, 0x0048, 0x0004, 0xA800);

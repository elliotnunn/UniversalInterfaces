...
typedef CALLBACK_API( ComponentResult , MusicMIDISendProcPtr )(MusicComponent self, long refCon, MusicMIDIPacket *mmp);
...
#endif
EXTERN_API( ComponentResult )
NAPickInstrument				(NoteAllocator 			ci,
...
EXTERN_API( ComponentResult )
NAPickArrangement				(NoteAllocator 			ci,
								 ModalFilterUPP 		filterProc,
								 StringPtr 				prompt,
								 long 					zero1,
								 long 					zero2,
...
EXTERN_API( ComponentResult )
NAGetIndNoteChannel				(NoteAllocator 			ci,
								 long 					index,
								 NoteChannel *			nc,
...
EXTERN_API( ComponentResult )
NAGetMIDIPorts					(NoteAllocator 			ci,
								 Handle *				inputPorts,
								 Handle *				outputPorts)						FIVEWORDINLINE(0x2F3C, 0x0008, 0x0021, 0x7000, 0xA82A);

EXTERN_API( ComponentResult )
NAGetNoteRequest				(NoteAllocator 			ci,
								 NoteChannel 			noteChannel,
								 NoteRequest *			nrOut)								FIVEWORDINLINE(0x2F3C, 0x0008, 0x0022, 0x7000, 0xA82A);

EXTERN_API( ComponentResult )
NASendMIDI						(NoteAllocator 			ci,
								 NoteChannel 			noteChannel,
								 MusicMIDIPacket *		mp)									FIVEWORDINLINE(0x2F3C, 0x0008, 0x0023, 0x7000, 0xA82A);

EXTERN_API( ComponentResult )
NAPickEditInstrument			(NoteAllocator 			ci,
								 ModalFilterUPP 		filterProc,
								 StringPtr 				prompt,
								 long 					refCon,
								 NoteChannel 			nc,
...
EXTERN_API( ComponentResult )
NAGetKnob						(NoteAllocator 			ci,
								 NoteChannel 			noteChannel,
								 long 					knobNumber,
								 long *					knobValue)							FIVEWORDINLINE(0x2F3C, 0x000C, 0x0028, 0x7000, 0xA82A);

EXTERN_API( ComponentResult )
NATask							(NoteAllocator 			ci)									FIVEWORDINLINE(0x2F3C, 0x0000, 0x0029, 0x7000, 0xA82A);

EXTERN_API( ComponentResult )
NASetNoteChannelBalance			(NoteAllocator 			ci,
								 NoteChannel 			noteChannel,
								 long 					balance)							FIVEWORDINLINE(0x2F3C, 0x0008, 0x002A, 0x7000, 0xA82A);

EXTERN_API( ComponentResult )
NASetInstrumentNumberInterruptSafe (NoteAllocator 		ci,
								 NoteChannel 			noteChannel,
								 long 					instrumentNumber)					FIVEWORDINLINE(0x2F3C, 0x0008, 0x002B, 0x7000, 0xA82A);

EXTERN_API( ComponentResult )
NASetNoteChannelSoundLocalization (NoteAllocator 		ci,
								 NoteChannel 			noteChannel,
...
enum {
	kTunePlayerType				= FOUR_CHAR_CODE('tune')
};
...
TuneInstant						(TunePlayer 			tp,
								 void *					tune,
								 long 					tunePosition)						FIVEWORDINLINE(0x2F3C, 0x0008, 0x000B, 0x7000, 0xA82A);
...
#define qtma_StuffGeneralEvent(w1,w2,part,subType,length)											w1 =	(kGeneralEventType << kXEventTypeFieldPos)						|	((long)(part) << kXEventPartFieldPos)							|	((long)(length) << kGeneralEventLengthFieldPos),				w2 =	(kGeneralEventLengthBits << kEventLengthFieldPos)				|	((long)(subType) << kGeneralEventSubtypeFieldPos)				|	((long)(length) << kGeneralEventLengthFieldPos)

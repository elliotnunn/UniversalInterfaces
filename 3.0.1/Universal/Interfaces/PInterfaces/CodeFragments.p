...
TYPE
	CFragConnectionID = ^LONGINT;
	CFragClosureID = ^LONGINT;
	CFragContainerID = ^LONGINT;
	CFragContextID = ^LONGINT;
...
CONST
	...
	kPrivateCFragCopy			= $0005;						{  Prepare a new private copy. }
...
FUNCTION GetSharedLibrary(libName: ConstStr63Param; archType: CFragArchitecture; loadFlags: CFragLoadOptions; VAR connID: CFragConnectionID; VAR mainAddr: Ptr; VAR errMessage: Str255): OSErr;
...
{  ! This must have a file specification at the same offset as a disk flat locator! }
	...
	CFragInitFunction = ProcPtr;  { FUNCTION CFragInitFunction((CONST)VAR initBlock: CFragInitBlock): OSErr; C; }

	CFragTermProcedure = ProcPtr;  { PROCEDURE CFragTermProcedure; C; }
...
CONST
	kLoadCFrag					= $0001;						{  ! Keep kLoadCFrag visible to compilers! }

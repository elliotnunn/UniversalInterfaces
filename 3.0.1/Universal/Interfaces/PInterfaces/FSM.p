...
TYPE
	...
	HFSCIRec = RECORD
		...
		compInterfProc:			HFSCIProcPtr;							{  pointer to file system call processing code  }
		log2PhyProc:			Lg2PhysProcPtr;							{  pointer to Lg2PhysProc() code  }
...
TYPE
	...
	DICIRec = RECORD
		compInterfMask:			LONGINT;								{  component flags  }
		compInterfProc:			DICIProcPtr;							{  pointer to call processing code  }
...
FUNCTION UTBlockInFQHashP(vRefNum: INTEGER; diskBlock: LONGINT): OSErr;
	...
	INLINE $702C, $A824;
	{$ENDC}
FUNCTION UTVolCacheReadIP(volCtrlBlockPtr: VCBPtr; blockPosition: LONGINT; ioBuffer: Ptr; reqCount: LONGINT; VAR actCount: LONGINT; cacheOption: INTEGER): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $7034, $A824;
	{$ENDC}
FUNCTION UTVolCacheWriteIP(volCtrlBlockPtr: VCBPtr; blockPosition: LONGINT; ioBuffer: Ptr; reqCount: LONGINT; VAR actCount: LONGINT; cacheOption: INTEGER): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $7035, $A824;
	{$ENDC}

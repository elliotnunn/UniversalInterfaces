...
TYPE
	...
	gxTag = ^LONGINT;
	gxColorSet = ^LONGINT;
	gxColorProfile = ^LONGINT;
	gxGraphicsClient = ^LONGINT;
	gxViewGroup = ^LONGINT;
	gxViewPort = ^LONGINT;
	gxViewDevice = ^LONGINT;
...
{ gxInk structures }
	...
	gxTransferComponent = RECORD
		mode:					SInt8;									{  how the component is operated upon  }
		flags:					SInt8;									{  flags for each component	 }
...
TYPE
	...
	gxSpoolProcPtr = ProcPtr;  { FUNCTION gxSpool(command: gxSpoolCommand; VAR block: gxSpoolBlock): LONGINT; C; }
	...
	gxSpoolBlock = RECORD
		...
		lastTypeOpcode:			SInt8;									{  type of last created object  }
		currentOperation:		SInt8;									{  operation emitted by flatten, or intrepreted by last unflatten  }
		currentOperand:			SInt8;									{  e.g., gxTransformTypeOpcode, gxInkTagOpcode  }

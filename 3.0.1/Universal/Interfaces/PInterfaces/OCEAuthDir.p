...
{ Fix parameter passing convention (#1274062) ggs, 8-7-95 }
	AuthIOCompletionProcPtr = Register68kProcPtr;  { PROCEDURE AuthIOCompletion(paramBlock: AuthParamBlockPtr); }
...
****************************************************************************}
	...
	DirIOCompletionProcPtr = Register68kProcPtr;  { PROCEDURE DirIOCompletion(paramBlock: DirParamBlockPtr); }
...
can extract the results in the 'getBuffer' by making DirEnumerateParse' call.
	...
	DirEnumerateGetPB = RECORD
		...
		padByte:				SInt8;
		matchNameHow:			SInt8;									{   --> Matching Criteria for nameMatchString  }
		matchTypeHow:			SInt8;									{   --> Matching Criteria for typeList  }
		getBuffer:				Ptr;									{   -->  }
...
This call can be done only in synchronous mode.
	...
	DSAMDirProcPtr = ProcPtr;  { FUNCTION DSAMDir(dsamData: UNIV Ptr; paramBlock: DirParamBlockPtr; async: BOOLEAN): OSErr; }

	DSAMDirUPP = UniversalProcPtr;
	DSAMDirProc							= DSAMDirUPP;
	DSAMDirParseProcPtr = ProcPtr;  { FUNCTION DSAMDirParse(dsamData: UNIV Ptr; paramBlock: DirParamBlockPtr; async: BOOLEAN): OSErr; }

	DSAMDirParseUPP = UniversalProcPtr;
	DSAMDirParseProc					= DSAMDirParseUPP;
	DSAMAuthProcPtr = ProcPtr;  { FUNCTION DSAMAuth(dsamData: UNIV Ptr; pb: AuthParamBlockPtr; async: BOOLEAN): OSErr; }

...
TYPE
	...
	TEPtr								= ^TERec;
	TEHandle							= ^TEPtr;
	HighHookProcPtr = Register68kProcPtr;  { PROCEDURE HighHook((CONST)VAR r: Rect; pTE: TEPtr); }

	EOLHookProcPtr = Register68kProcPtr;  { FUNCTION EOLHook(theChar: ByteParameter; pTE: TEPtr; hTE: TEHandle): BOOLEAN; }

	CaretHookProcPtr = Register68kProcPtr;  { PROCEDURE CaretHook((CONST)VAR r: Rect; pTE: TEPtr); }

	WidthHookProcPtr = Register68kProcPtr;  { FUNCTION WidthHook(textLen: INTEGER; textOffset: INTEGER; textBufferPtr: UNIV Ptr; pTE: TEPtr; hTE: TEHandle): INTEGER; }

	TextWidthHookProcPtr = Register68kProcPtr;  { FUNCTION TextWidthHook(textLen: INTEGER; textOffset: INTEGER; textBufferPtr: UNIV Ptr; pTE: TEPtr; hTE: TEHandle): INTEGER; }

	NWidthHookProcPtr = Register68kProcPtr;  { FUNCTION NWidthHook(styleRunLen: INTEGER; styleRunOffset: INTEGER; slop: INTEGER; direction: INTEGER; textBufferPtr: UNIV Ptr; VAR lineStart: INTEGER; pTE: TEPtr; hTE: TEHandle): INTEGER; }

	DrawHookProcPtr = Register68kProcPtr;  { PROCEDURE DrawHook(textOffset: INTEGER; drawLen: INTEGER; textBufferPtr: UNIV Ptr; pTE: TEPtr; hTE: TEHandle); }

	HitTestHookProcPtr = Register68kProcPtr;  { FUNCTION HitTestHook(styleRunLen: INTEGER; styleRunOffset: INTEGER; slop: INTEGER; textBufferPtr: UNIV Ptr; pTE: TEPtr; hTE: TEHandle; VAR pixelWidth: INTEGER; VAR charOffset: INTEGER; VAR pixelInChar: BOOLEAN): BOOLEAN; }

	TEFindWordProcPtr = Register68kProcPtr;  { PROCEDURE TEFindWord(currentPos: INTEGER; caller: INTEGER; pTE: TEPtr; hTE: TEHandle; VAR wordStart: INTEGER; VAR wordEnd: INTEGER); }

	TERecalcProcPtr = Register68kProcPtr;  { PROCEDURE TERecalc(pTE: TEPtr; changeLength: INTEGER; VAR lineStart: INTEGER; VAR firstChar: INTEGER; VAR lastChar: INTEGER); }

	TEDoTextProcPtr = Register68kProcPtr;  { PROCEDURE TEDoText(pTE: TEPtr; firstChar: INTEGER; lastChar: INTEGER; selector: INTEGER; VAR currentGrafPort: GrafPtr; VAR charPosition: INTEGER); }

	TEClickLoopProcPtr = Register68kProcPtr;  { FUNCTION TEClickLoop(pTE: TEPtr): BOOLEAN; }

	WordBreakProcPtr = Register68kProcPtr;  { FUNCTION WordBreak(text: Ptr; charPos: INTEGER): BOOLEAN; }
...
CONST
	...
	teFInlineInput				= 3;							{ 00001000b  }
																{ 	teFUseTextServices is obsolete.  }

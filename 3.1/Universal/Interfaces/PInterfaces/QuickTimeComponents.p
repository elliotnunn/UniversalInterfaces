...
{$IFC TYPED_FUNCTION_POINTERS}
	SCModalFilterProcPtr = FUNCTION(theDialog: DialogPtr; VAR theEvent: EventRecord; VAR itemHit: INTEGER; refcon: LONGINT): BOOLEAN;
{$ELSEC}
	SCModalFilterProcPtr = ProcPtr;
{$ENDC}

{$IFC TYPED_FUNCTION_POINTERS}
	SCModalHookProcPtr = FUNCTION(theDialog: DialogPtr; itemHit: INTEGER; params: UNIV Ptr; refcon: LONGINT): INTEGER;
{$ELSEC}
...
{$IFC TYPED_FUNCTION_POINTERS}
	TweenerDataProcPtr = FUNCTION(VAR tr: TweenRecord; tweenData: UNIV Ptr; tweenDataSize: LONGINT; dataDescriptionSeed: LONGINT; dataDescription: Handle; asyncCompletionProc: ICMCompletionProcRecordPtr; transferProc: UniversalProcPtr; refCon: UNIV Ptr): ComponentResult;
{$ELSEC}
...
{  QTAtomTypes for atoms in import/export settings containers }
	...
	kQTSettingsTextDescription	= 'desc';						{  Text settings (TextDescription record) }
	kQTSettingsTextSize			= 'size';						{  Width/height to create (FixedPoint) }
...
FUNCTION NewMovieExportGetDataProc(userRoutine: MovieExportGetDataProcPtr): MovieExportGetDataUPP;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}

FUNCTION NewMovieExportGetPropertyProc(userRoutine: MovieExportGetPropertyProcPtr): MovieExportGetPropertyUPP;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}
...
FUNCTION CallTweenerDataProc(VAR tr: TweenRecord; tweenData: UNIV Ptr; tweenDataSize: LONGINT; dataDescriptionSeed: LONGINT; dataDescription: Handle; asyncCompletionProc: ICMCompletionProcRecordPtr; transferProc: UniversalProcPtr; refCon: UNIV Ptr; userRoutine: TweenerDataUPP): ComponentResult;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}

FUNCTION CallMovieExportGetDataProc(refCon: UNIV Ptr; VAR params: MovieExportGetDataParams; userRoutine: MovieExportGetDataUPP): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}

FUNCTION CallMovieExportGetPropertyProc(refcon: UNIV Ptr; trackID: LONGINT; propertyType: OSType; propertyValue: UNIV Ptr; userRoutine: MovieExportGetPropertyUPP): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}
...
TYPE
{$IFC TYPED_FUNCTION_POINTERS}
	SGModalFilterProcPtr = FUNCTION(theDialog: DialogPtr; {CONST}VAR theEvent: EventRecord; VAR itemHit: INTEGER; refCon: LONGINT): BOOLEAN;
{$ELSEC}
...
FUNCTION SGPanelInstall(s: SeqGrabComponent; c: SGChannel; d: DialogPtr; itemOffset: INTEGER): ComponentResult;
	...
	{$ENDC}
FUNCTION SGPanelEvent(s: SeqGrabComponent; c: SGChannel; d: DialogPtr; itemOffset: INTEGER; {CONST}VAR theEvent: EventRecord; VAR itemHit: INTEGER; VAR handled: BOOLEAN): ComponentResult;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
{  Display mode atom types }
	...
	kQTVOName					= 'name';						{  atom contains string (no length byte) - name of modefor display to user }
...
FUNCTION CallSGModalFilterProc(theDialog: DialogPtr; {CONST}VAR theEvent: EventRecord; VAR itemHit: INTEGER; refCon: LONGINT; userRoutine: SGModalFilterUPP): BOOLEAN;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}

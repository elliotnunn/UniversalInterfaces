...
typedef CALLBACK_API( void , ThemeButtonDrawProcPtr )(const Rect *bounds, ThemeButtonKind kind, ThemeButtonDrawInfo *info, UInt32 userData, SInt16 depth, Boolean isColorDev);
...
#define NewThemeTabTitleDrawProc(userRoutine) 					(ThemeTabTitleDrawUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppThemeTabTitleDrawProcInfo, GetCurrentArchitecture())
#define NewThemeEraseProc(userRoutine) 							(ThemeEraseUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppThemeEraseProcInfo, GetCurrentArchitecture())
#define NewThemeButtonDrawProc(userRoutine) 					(ThemeButtonDrawUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppThemeButtonDrawProcInfo, GetCurrentArchitecture())
#define NewWindowTitleDrawingProc(userRoutine) 					(WindowTitleDrawingUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppWindowTitleDrawingProcInfo, GetCurrentArchitecture())
#define NewThemeIteratorProc(userRoutine) 						(ThemeIteratorUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppThemeIteratorProcInfo, GetCurrentArchitecture())
#define CallThemeTabTitleDrawProc(userRoutine, bounds, style, direction, depth, isColorDev, userData)  CALL_SIX_PARAMETER_UPP((userRoutine), uppThemeTabTitleDrawProcInfo, (bounds), (style), (direction), (depth), (isColorDev), (userData))
#define CallThemeEraseProc(userRoutine, bounds, eraseData, depth, isColorDev)  CALL_FOUR_PARAMETER_UPP((userRoutine), uppThemeEraseProcInfo, (bounds), (eraseData), (depth), (isColorDev))
#define CallThemeButtonDrawProc(userRoutine, bounds, kind, info, userData, depth, isColorDev)  CALL_SIX_PARAMETER_UPP((userRoutine), uppThemeButtonDrawProcInfo, (bounds), (kind), (info), (userData), (depth), (isColorDev))
#define CallWindowTitleDrawingProc(userRoutine, bounds, depth, colorDevice, userData)  CALL_FOUR_PARAMETER_UPP((userRoutine), uppWindowTitleDrawingProcInfo, (bounds), (depth), (colorDevice), (userData))
#define CallThemeIteratorProc(userRoutine, inFileName, resID, inThemeSettings, inUserData)  CALL_FOUR_PARAMETER_UPP((userRoutine), uppThemeIteratorProcInfo, (inFileName), (resID), (inThemeSettings), (inUserData))
/*——————————————————————————————————————————————————————————————————————————————————*/

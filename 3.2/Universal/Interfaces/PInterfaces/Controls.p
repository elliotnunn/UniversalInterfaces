...
{——————————————————————————————————————————————————————————————————————————————————————————————————————}
	...
	gestaltControlMgrPresent	= $00000001;
...
{ font ignores all other flags 											}
	kControlAddToMetaFontMask	= $0200;
...
{ Features introduced in Appearance 1.1 }
	kControlAutoToggles			= $4000;
	kControlSupportsGetRegion	= $00020000;
...
FUNCTION NewControlEditTextValidationProc(userRoutine: ControlEditTextValidationProcPtr): ControlEditTextValidationUPP;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}

PROCEDURE CallControlEditTextValidationProc(control: ControlHandle; userRoutine: ControlEditTextValidationUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}
{——————————————————————————————————————————————————————————————————————————————————————}
...
FUNCTION FindControlUnderMouse(inWhere: Point; inWindow: WindowPtr; VAR outPart: SInt16): ControlHandle;
	...
	{$ENDC}
FUNCTION HandleControlClick(inControl: ControlHandle; inWhere: Point; inModifiers: SInt16; inAction: ControlActionUPP): ControlPartCode;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION GetControlViewSize(theControl: ControlHandle): SInt32;
PROCEDURE SetControlViewSize(theControl: ControlHandle; newViewSize: SInt32);
FUNCTION GetControl32BitValue(theControl: ControlHandle): SInt32;
PROCEDURE SetControl32BitValue(theControl: ControlHandle; newValue: SInt32);
FUNCTION GetControl32BitMaximum(theControl: ControlHandle): SInt32;
PROCEDURE SetControl32BitMaximum(theControl: ControlHandle; newMaximum: SInt32);
FUNCTION GetControl32BitMinimum(theControl: ControlHandle): SInt32;
PROCEDURE SetControl32BitMinimum(theControl: ControlHandle; newMinimum: SInt32);
{
...
FUNCTION IsValidControlHandle(theControl: ControlHandle): BOOLEAN;
{——————————————————————————————————————————————————————————————————————————————————————}
{ • Properties																			}
{——————————————————————————————————————————————————————————————————————————————————————}
FUNCTION GetControlProperty(control: ControlHandle; propertyCreator: OSType; propertyTag: OSType; bufferSize: UInt32; VAR actualSize: UInt32; propertyBuffer: UNIV Ptr): OSStatus;
FUNCTION GetControlPropertySize(control: ControlHandle; propertyCreator: OSType; propertyTag: OSType; VAR size: UInt32): OSStatus;
FUNCTION SetControlProperty(control: ControlHandle; propertyCreator: OSType; propertyTag: OSType; propertySize: UInt32; propertyData: UNIV Ptr): OSStatus;
FUNCTION RemoveControlProperty(control: ControlHandle; propertyCreator: OSType; propertyTag: OSType): OSStatus;
...
{——————————————————————————————————————————————————————————————————————————————————————}
FUNCTION GetControlRegion(inControl: ControlHandle; inPart: ControlPartCode; outRegion: RgnHandle): OSStatus;
...
FUNCTION GetSuperControl(inControl: ControlHandle; VAR outParent: ControlHandle): OSErr;
	...
	{$ENDC}
FUNCTION CountSubControls(inControl: ControlHandle; VAR outNumChildren: UInt16): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $303C, $0016, $AA73;
	{$ENDC}
FUNCTION GetIndexedSubControl(inControl: ControlHandle; inIndex: UInt16; VAR outSubControl: ControlHandle): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}

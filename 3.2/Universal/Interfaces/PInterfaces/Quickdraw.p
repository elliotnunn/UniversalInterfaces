...
FUNCTION NewQDStdGlyphsProc(userRoutine: QDStdGlyphsProcPtr): QDStdGlyphsUPP;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}
...
FUNCTION CallQDStdGlyphsProc(dataStream: UNIV Ptr; size: ByteCount; userRoutine: QDStdGlyphsUPP): OSStatus;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
{$IFC NOT OLDPIXMAPSTRUCT }

TYPE
	PixMapExtensionPtr = ^PixMapExtension;
	PixMapExtension = RECORD
		extSize:				LONGINT;								{ size of struct, duh! }
		pmBits:					UInt32;									{ pixmap attributes bitfield }
		pmGD:					Ptr;									{ this is a GDHandle }
		pmSeed:					LONGINT;
		reserved0:				UInt32;									{ reserved for future use }
		reserved1:				UInt32;
		reserved2:				UInt32;
		longRowBytes:			LONGINT;								{ used when rowBytes > 16382 }
	END;

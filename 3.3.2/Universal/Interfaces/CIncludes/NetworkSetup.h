...
#if CALL_NOT_IN_CARBON
EXTERN_API( Handle )
OTCfgGetDefault                 (OSType                 entityType,
                                 OSType                 entityClass,
                                 OSType                 prefsType);
...
/*
    ...
    Inputs:     CfgDatabaseRef dbRef            Reference to opened database
                ...
                OTNotifyProcPtr notifier        address of callback routine
    ...
    Requires Network Setup 1.0.2 or higher.
*/
EXTERN_API( OSStatus )
...
enum {
    kOTCfgDatabaseChanged       = kPRIVATEEVENT + 0             /* result will be kCfgErrDatabaseChanged, cookie is meaningless*/
};
...
struct OTCfgATalkGeneral {
    ...
    OTPortRef                       fPort;                      /* a reference to the port over which this configuration is running*/
...
struct OTCfgIRGeneral {
    UInt32                          fVersion;                   /* must be 0*/
    OTPortRef                       fPortRef;                   /* reference to IR port*/
...
struct OTCfgRemoteLogOptions {
    ...
    UInt32                          reserved[4];                /* must be zero*/
};
...
struct OTCfgRemoteServerPort {
    ...
    InetHost                        localIPAddress;             /* IP address to offer to client*/
};

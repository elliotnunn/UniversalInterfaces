...
/*
   ••• I changed all the routines that required size_t
   to use ByteCount instead.  But that causes a conflict with
   OT internal builds, because ByteCount is of type (unsigned long)
   and, for OT internal builds, size_t is of type (unsigned int).
   This problem is happens for a number of other like types.  The
   solution is to define OTByteCount, which is always a UInt32,
   but is defined as either ByteCount (an unsigned long) for two-byte
   int compilers and an unsigned int for four-byte int compilers.
*/
...
/* Some of the following contants conflict with BSD system headers */
enum {
    kSIGHUP                     = 1,
    kSIGURG                     = 16,
    kSIGPOLL                    = 30
...
typedef TVECTOR_UPP_TYPE(OTNotifyProcPtr)                       OTNotifyUPP;
    ...
    EXTERN_API(void)
    InvokeOTNotifyUPP              (void *                  contextPtr,
                                    OTEventCode             code,
                                    OTResult                result,
                                    void *                  cookie,
                                    OTNotifyUPP             userUPP);
...
enum {
    T_UNSPEC                    = (unsigned long)0xFFFFFFFD,
    T_ALLOPT                    = 0
};
...
OTCreatePortRef                 (OTBusType              busType,
...
OTGetSlotFromPortRef            (OTPortRef              ref,
                                 UInt16 *               other);

EXTERN_API( OTPortRef )
OTSetDeviceTypeInPortRef        (OTPortRef              ref,
                                 OTDeviceType           devType);

EXTERN_API( OTPortRef )
OTSetBusTypeInPortRef           (OTPortRef              ref,
                                 OTBusType              busType);
...
#if !OTKERNEL
EXTERN_API( Boolean )
OTGetIndexedPort                (OTPortRecord *         portRecord,
                                 OTItemCount            index);

/* Index through the ports in the system*/
EXTERN_API( Boolean )
OTFindPort                      (OTPortRecord *         portRecord,
                                 const char *           portName);

/* Find an OTPortRecord for a port using it's name*/
EXTERN_API( Boolean )
OTFindPortByRef                 (OTPortRecord *         portRecord,
                                 OTPortRef              ref);

/* Find an OTPortRecord for a port using it's OTPortRef*/
#if CALL_NOT_IN_CARBON
EXTERN_API_C( OSStatus )
OTRegisterPort                  (OTPortRecord *         portRecord,
                                 void *                 ref);
...
EXTERN_API( OSStatus )
InitOpenTransportInContext      (OTInitializationFlags  flags,
...
EXTERN_API( OSStatus )
OTRegisterAsClient              (OTClientName           name,
                                 OTNotifyUPP            proc);
...
*/
EXTERN_API( long )
OTCreateDeferredTaskInContext   (OTProcessUPP           upp,
                                 void *                 arg,
...
OTCreateDeferredTask            (OTProcessUPP           proc,
                                 void *                 arg);
...
OTCreateSystemTask              (OTProcessUPP           proc,
                                 void *                 arg);
...
#if CALL_NOT_IN_CARBON
EXTERN_API( OSStatus )
OTAsyncOpenProvider             (OTConfigurationRef     cfig,
                                 OTOpenFlags            flags,
                                 OTNotifyUPP            proc,
                                 void *                 contextPtr);

EXTERN_API( ProviderRef )
OTOpenProvider                  (OTConfigurationRef     cfig,
                                 OTOpenFlags            flags,
                                 OSStatus *             errPtr);
...
#if CALL_NOT_IN_CARBON
EXTERN_API( ProviderRef )
OTTransferProviderOwnership     (ProviderRef            ref,
                                 OTClient               prevOwner,
                                 OSStatus *             errPtr);
...
EXTERN_API( SInt32 )
OTIoctl                         (ProviderRef            ref,
                                 UInt32                 cmd,
                                 void *                 data);

#if CALL_NOT_IN_CARBON
EXTERN_API( OTResult )
OTGetMessage                    (ProviderRef            ref,
                                 strbuf *               ctlbuf,
                                 strbuf *               databuf,
                                 OTFlags *              flagsPtr);

EXTERN_API( OTResult )
OTGetPriorityMessage            (ProviderRef            ref,
                                 strbuf *               ctlbuf,
                                 strbuf *               databuf,
                                 OTBand *               bandPtr,
                                 OTFlags *              flagsPtr);
...
EXTERN_API( OSStatus )
OTPutMessage                    (ProviderRef            ref,
                                 const strbuf *         ctlbuf,
                                 const strbuf *         databuf,
                                 OTFlags                flags);

EXTERN_API( OSStatus )
OTPutPriorityMessage            (ProviderRef            ref,
                                 const strbuf *         ctlbuf,
                                 const strbuf *         databuf,
                                 OTBand                 band,
                                 OTFlags                flags);
...
EXTERN_API( OSStatus )
OTInstallNotifier               (ProviderRef            ref,
                                 OTNotifyUPP            proc,
                                 void *                 contextPtr);

EXTERN_API( OSStatus )
OTUseSyncIdleEvents             (ProviderRef            ref,
                                 Boolean                useEvents);
...
EXTERN_API( OSStatus )
OTCancelSynchronousCalls        (ProviderRef            ref,
                                 OSStatus               err);
...
OTOpenEndpointInContext         (OTConfigurationRef     config,
                                 OTOpenFlags            oflag,
...
EXTERN_API( OSStatus )
OTAsyncOpenEndpointInContext    (OTConfigurationRef     config,
                                 OTOpenFlags            oflag,
                                 TEndpointInfo *        info, /* can be NULL */
                                 OTNotifyUPP            upp,
                                 void *                 contextPtr,
...
#if CALL_NOT_IN_CARBON
EXTERN_API( EndpointRef )
OTOpenEndpoint                  (OTConfigurationRef     cfig,
                                 OTOpenFlags            oflag,
                                 TEndpointInfo *        info, /* can be NULL */
                                 OSStatus *             err);

EXTERN_API( OSStatus )
OTAsyncOpenEndpoint             (OTConfigurationRef     cfig,
                                 OTOpenFlags            oflag,
                                 TEndpointInfo *        info, /* can be NULL */
                                 OTNotifyUPP            proc,
                                 void *                 contextPtr);
...
EXTERN_API( OSStatus )
OTGetEndpointInfo               (EndpointRef            ref,
                                 TEndpointInfo *        info);
...
EXTERN_API( OTResult )
OTCountDataBytes                (EndpointRef            ref,
                                 OTByteCount *          countPtr);

EXTERN_API( OSStatus )
OTGetProtAddress                (EndpointRef            ref,
                                 TBind *                boundAddr,
                                 TBind *                peerAddr);

EXTERN_API( OSStatus )
OTResolveAddress                (EndpointRef            ref,
                                 TBind *                reqAddr,
                                 TBind *                retAddr,
                                 OTTimeout              timeOut);
...
*/
EXTERN_API( void *)
OTAllocInContext                (EndpointRef            ref,
...
EXTERN_API( OSStatus )
OTOptionManagement              (EndpointRef            ref,
                                 TOptMgmt *             req,
                                 TOptMgmt *             ret);
...
#if CALL_NOT_IN_CARBON
EXTERN_API( OSStatus )
OTCreateOptions                 (const char *           endPtName,
                                 char **                strPtr,
                                 TNetbuf *              buf);

EXTERN_API( OSStatus )
OTCreateOptionString            (const char *           endPtName,
                                 TOption **             opt,
                                 void *                 bufEnd,
                                 char *                 str,
...
EXTERN_API( OSStatus )
OTNextOption                    (UInt8 *                buffer,
                                 UInt32                 buflen,
                                 TOption **             prevOptPtr);

EXTERN_API( TOption *)
OTFindOption                    (UInt8 *                buffer,
                                 UInt32                 buflen,
                                 OTXTILevel             level,
                                 OTXTIName              name);
...
EXTERN_API( OSStatus )
OTConnect                       (EndpointRef            ref,
                                 TCall *                sndCall,
                                 TCall *                rcvCall) /* can be NULL */;

EXTERN_API( OSStatus )
OTRcvConnect                    (EndpointRef            ref,
                                 TCall *                call) /* can be NULL */;

EXTERN_API( OSStatus )
OTListen                        (EndpointRef            ref,
                                 TCall *                call);

EXTERN_API( OSStatus )
OTAccept                        (EndpointRef            listener,
                                 EndpointRef            worker,
                                 TCall *                call);

EXTERN_API( OSStatus )
OTSndDisconnect                 (EndpointRef            ref,
                                 TCall *                call);
...
EXTERN_API( OSStatus )
OTRcvDisconnect                 (EndpointRef            ref,
...
EXTERN_API( OTResult )
OTRcv                           (EndpointRef            ref,
                                 void *                 buf,
                                 OTByteCount            nbytes,
                                 OTFlags *              flags);

EXTERN_API( OTResult )
OTSnd                           (EndpointRef            ref,
                                 void *                 buf,
                                 OTByteCount            nbytes,
                                 OTFlags                flags);
...
EXTERN_API( OSStatus )
OTSndUData                      (EndpointRef            ref,
                                 TUnitData *            udata);

EXTERN_API( OSStatus )
OTRcvUData                      (EndpointRef            ref,
                                 TUnitData *            udata,
                                 OTFlags *              flags);

EXTERN_API( OSStatus )
OTRcvUDErr                      (EndpointRef            ref,
                                 TUDErr *               uderr);
...
#if CALL_NOT_IN_CARBON
EXTERN_API( OSStatus )
OTSndRequest                    (EndpointRef            ref,
                                 TRequest *             req,
                                 OTFlags                reqFlags);

EXTERN_API( OSStatus )
OTRcvReply                      (EndpointRef            ref,
                                 TReply *               reply,
                                 OTFlags *              replyFlags);

EXTERN_API( OSStatus )
OTSndReply                      (EndpointRef            ref,
                                 TReply *               reply,
                                 OTFlags                replyFlags);

EXTERN_API( OSStatus )
OTRcvRequest                    (EndpointRef            ref,
                                 TRequest *             req,
                                 OTFlags *              reqFlags);

EXTERN_API( OSStatus )
OTCancelRequest                 (EndpointRef            ref,
                                 OTSequence             sequence);

EXTERN_API( OSStatus )
OTCancelReply                   (EndpointRef            ref,
                                 OTSequence             sequence);

/* Connectionless transactions*/

EXTERN_API( OSStatus )
OTSndURequest                   (EndpointRef            ref,
                                 TUnitRequest *         req,
                                 OTFlags                reqFlags);

EXTERN_API( OSStatus )
OTRcvUReply                     (EndpointRef            ref,
                                 TUnitReply *           reply,
                                 OTFlags *              replyFlags);

EXTERN_API( OSStatus )
OTSndUReply                     (EndpointRef            ref,
                                 TUnitReply *           reply,
                                 OTFlags                replyFlags);

EXTERN_API( OSStatus )
OTRcvURequest                   (EndpointRef            ref,
                                 TUnitRequest *         req,
                                 OTFlags *              reqFlags);

EXTERN_API( OSStatus )
OTCancelURequest                (EndpointRef            ref,
                                 OTSequence             seq);

EXTERN_API( OSStatus )
OTCancelUReply                  (EndpointRef            ref,
                                 OTSequence             seq);
...
EXTERN_API( OSStatus )
OTAsyncOpenMapperInContext      (OTConfigurationRef     config,
                                 OTOpenFlags            oflag,
                                 OTNotifyUPP            upp,
                                 void *                 contextPtr,
                                 OTClientContextPtr     clientContext);
...
OTOpenMapperInContext           (OTConfigurationRef     config,
                                 OTOpenFlags            oflag,
                                 OSStatus *             err,
                                 OTClientContextPtr     clientContext);
...
#if CALL_NOT_IN_CARBON
EXTERN_API( OSStatus )
OTAsyncOpenMapper               (OTConfigurationRef     cfig,
                                 OTOpenFlags            oflag,
                                 OTNotifyUPP            proc,
                                 void *                 contextPtr);

EXTERN_API( MapperRef )
OTOpenMapper                    (OTConfigurationRef     cfig,
                                 OTOpenFlags            oflag,
                                 OSStatus *             err);
...
EXTERN_API( OSStatus )
OTRegisterName                  (MapperRef              ref,
                                 TRegisterRequest *     req,
                                 TRegisterReply *       reply);

EXTERN_API( OSStatus )
OTDeleteName                    (MapperRef              ref,
                                 TNetbuf *              name);

EXTERN_API( OSStatus )
OTDeleteNameByID                (MapperRef              ref,
                                 OTNameID               nameID);

EXTERN_API( OSStatus )
OTLookupName                    (MapperRef              ref,
                                 TLookupRequest *       req,
                                 TLookupReply *         reply);
...
OTSubtractTimeStamps            (OTTimeStamp *          result,
...
typedef TVECTOR_UPP_TYPE(OTListSearchProcPtr)                   OTListSearchUPP;
    ...
    EXTERN_API(Boolean)
    InvokeOTListSearchUPP          (const void *            ref,
                                    OTLink *                linkToCheck,
...
EXTERN_API_C( void )
OTAddFirst                      (OTList *               list,
                                 OTLink *               link);

/* Add the link to the list at the end*/
EXTERN_API_C( void )
OTAddLast                       (OTList *               list,
                                 OTLink *               link);
...
*/
                                                                                            ...
                                                                                            #endif
EXTERN_API_C( Boolean )
OTAtomicClearBit                (UInt8 *                bytePtr,
...
*/
                                                                                            ...
                                                                                            #endif
EXTERN_API_C( Boolean )
OTCompareAndSwapPtr             (void *                 oldValue,
                                 void *                 newValue,
...
*/
                                                                                            ...
                                                                                            #endif
EXTERN_API_C( Boolean )
OTCompareAndSwap32              (UInt32                 oldValue,
                                 UInt32                 newValue,
...
*/
                                                                                            ...
                                                                                            #endif
EXTERN_API_C( Boolean )
OTCompareAndSwap16              (UInt32                 oldValue,
                                 UInt32                 newValue,
...
*/
                                                                                            ...
                                                                                            #endif
EXTERN_API_C( Boolean )
OTCompareAndSwap8               (UInt32                 oldValue,
                                 UInt32                 newValue,
...
*/
EXTERN_API_C( SInt16 )
OTAtomicAdd16                   (SInt32                 toAdd,
                                 SInt16 *               dest);

/* Not used frequently enough to justify inlining.*/
EXTERN_API_C( SInt8 )
OTAtomicAdd8                    (SInt32                 toAdd,
                                 SInt8 *                dest);

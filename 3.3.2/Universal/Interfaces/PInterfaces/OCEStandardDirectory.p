...
{$IFC TYPED_FUNCTION_POINTERS}
	PanelBusyProcPtr = PROCEDURE(Panel: SDPPanelHandle; busy: BOOLEAN);
{$ELSEC}
	PanelBusyProcPtr = ProcPtr;
{$ENDC}
...
FUNCTION NewPanelBusyUPP(userRoutine: PanelBusyProcPtr): PanelBusyUPP; { old name was NewPanelBusyProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}

PROCEDURE DisposePanelBusyUPP(userUPP: PanelBusyUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}

PROCEDURE InvokePanelBusyUPP(Panel: SDPPanelHandle; busy: BOOLEAN; userRoutine: PanelBusyUPP); { old name was CallPanelBusyProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}
...
TYPE
	SDPFindPanelResult					= UInt16;
	PackedRStringListHandle				= ^PackedPathNamePtr;
{$IFC TYPED_FUNCTION_POINTERS}
	FindPanelBusyProcPtr = PROCEDURE(findPanel: SDPFindPanelHandle; busy: BOOLEAN);
{$ELSEC}
	FindPanelBusyProcPtr = ProcPtr;
{$ENDC}
...
FUNCTION NewFindPanelBusyUPP(userRoutine: FindPanelBusyProcPtr): FindPanelBusyUPP; { old name was NewFindPanelBusyProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}

PROCEDURE DisposeFindPanelBusyUPP(userUPP: FindPanelBusyUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}

PROCEDURE InvokeFindPanelBusyUPP(findPanel: SDPFindPanelHandle; busy: BOOLEAN; userRoutine: FindPanelBusyUPP); { old name was CallFindPanelBusyProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}
{$ENDC}  {CALL_NOT_IN_CARBON}
...
{$IFC CALL_NOT_IN_CARBON }
FUNCTION SDPPromptForID(VAR id: AuthIdentity; guestPrompt: Str255; specificIDPrompt: Str255; localIDPrompt: Str255; {CONST}VAR recordType: RString; permittedKinds: SDPIdentityKind; VAR selectedKind: SDPIdentityKind; {CONST}VAR loginFilter: RecordID; filterKind: SDPLoginFilterKind): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION SDPNewPanel(VAR newPanel: SDPPanelHandle; window: WindowPtr; {CONST}VAR bounds: Rect; visible: BOOLEAN; enabled: BOOLEAN; {CONST}VAR initialRLI: PackedRLI; {CONST}VAR typesList: RStringPtr; typeCount: UInt32; identity: AuthIdentity; enumFlags: DirEnumChoices; matchTypeHow: DirMatchWith; refCon: LONGINT): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION SDPGetPanelSelectionSize(panel: SDPPanelHandle; VAR dsSpecSize: UInt16): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION SDPGetPathLength(panel: SDPPanelHandle; VAR pathNameLength: UInt16): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION SDPGetFindPanelState(findPanel: SDPFindPanelHandle; VAR itsState: SDPFindPanelState): OSErr;
	...
	{$ENDC}
FUNCTION SDPGetFindPanelSelectionSize(findPanel: SDPFindPanelHandle; VAR size: UInt16): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}

...
TYPE
	...
	GrafPort = RECORD
		...
		pnLoc:					Point;									{  in Carbon use Get/SetPortPenLocation }
...
{$IFC NOT OPAQUE_TOOLBOX_STRUCTS }
	CGrafPort = RECORD
		...
		fillPixPat:				PixPatHandle;							{  in Carbon use Get/SetPortFillPixPat }
		pnVis:					INTEGER;								{  in Carbon use GetPortPenVisibility or Show/HidePen }
		txFont:					INTEGER;								{  in Carbon use GetPortTextFont or TextFont }
		txFace:					StyleField;								{  in Carbon use GetPortTextFace or TextFace }
																		{ StyleField occupies 16-bits, but only first 8-bits are used }
...
{$IFC NOT TARGET_OS_MAC OR TARGET_API_MAC_OSX }
{$IFC CALL_NOT_IN_CARBON }
...
{ GetPortBitMapForCopyBits is provided for the specific purpose of using the return value as a parameter
       to CopyBits. The return value can be used as the srcBits or dstBits parameter to CopyBits regardless of
       whether the port is color. If the port parameter is a color port, however, the returned BitMapPtr does
       not actually point to a BitMap; it points to the PixMapHandle and other fields in the CGrafPort structure.
       You should not dereference the BitMapPtr or otherwise depend on its contents unless you've confirmed that
       this port is a non-color port. }

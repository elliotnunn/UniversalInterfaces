...
FUNCTION NewControl(owningWindow: WindowPtr; {CONST}VAR boundsRect: Rect; controlTitle: Str255; initiallyVisible: BOOLEAN; initialValue: SInt16; minimumValue: SInt16; maximumValue: SInt16; procID: SInt16; controlReference: SInt32): ControlRef;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $A954;
	{$ENDC}
FUNCTION GetNewControl(resourceID: SInt16; owningWindow: WindowPtr): ControlRef;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $A9BE;
	{$ENDC}
...
FUNCTION TestControl(theControl: ControlRef; testPoint: Point): ControlPartCode;
	...
	INLINE $A966;
	{$ENDC}
FUNCTION FindControl(testPoint: Point; theWindow: WindowPtr; VAR theControl: ControlRef): ControlPartCode;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $A96C;
	{$ENDC}
...
FUNCTION FindControlUnderMouse(inWhere: Point; inWindow: WindowPtr; VAR outPart: SInt16): ControlRef;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION DumpControlHierarchy(inWindow: WindowPtr; {CONST}VAR inDumpFile: FSSpec): OSErr;
	...
	{$ENDC}
FUNCTION CreateRootControl(inWindow: WindowPtr; VAR outControl: ControlRef): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $303C, $0001, $AA73;
	{$ENDC}
FUNCTION GetRootControl(inWindow: WindowPtr; VAR outControl: ControlRef): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION EmbedControl(inControl: ControlRef; inContainer: ControlRef): OSErr;
	...
	{$ENDC}
FUNCTION AutoEmbedControl(inControl: ControlRef; inWindow: WindowPtr): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
{——————————————————————————————————————————————————————————————————————————————————————}
FUNCTION GetKeyboardFocus(inWindow: WindowPtr; VAR outControl: ControlRef): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $303C, $000D, $AA73;
	{$ENDC}
FUNCTION SetKeyboardFocus(inWindow: WindowPtr; inControl: ControlRef; inPart: ControlFocusPart): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION SetAutomaticControlDragTrackingEnabledForWindow(theWindow: WindowPtr; tracks: BOOLEAN): OSStatus;
FUNCTION IsAutomaticControlDragTrackingEnabledForWindow(theWindow: WindowPtr; VAR tracks: BOOLEAN): OSStatus;
...
TYPE
{$IFC TYPED_FUNCTION_POINTERS}
	ControlNotificationProcPtr = PROCEDURE(theWindow: WindowPtr; theControl: ControlRef; notification: ControlNotification; param1: LONGINT; param2: LONGINT); C;
{$ELSEC}
...
FUNCTION GetControlOwner(control: ControlRef): WindowPtr;
FUNCTION GetControlDataHandle(control: ControlRef): Handle;
FUNCTION GetControlPopupMenuHandle(control: ControlRef): MenuHandle;
...
PROCEDURE SetControlPopupMenuHandle(control: ControlRef; popupMenu: MenuHandle);

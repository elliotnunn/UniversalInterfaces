...
{——————————————————————————————————————————————————————————————————————————————————}
{$IFC TARGET_CARBON }
{  So as to not conflict with Balloons.h on non-Carbon builds }
...
CONST
	...
	kHMHelpPropertyContentTag	= 'help';
...
{$IFC TYPED_FUNCTION_POINTERS}
	HMWindowContentProcPtr = FUNCTION(inWindow: WindowPtr; inGlobalMouse: Point; inRequest: HMContentRequest; VAR outContentProvided: HMContentProvidedType; ioHelpContent: HMHelpContentPtr): OSStatus;
{$ELSEC}
	HMWindowContentProcPtr = ProcPtr;
{$ENDC}

{$IFC TYPED_FUNCTION_POINTERS}
	HMMenuTitleContentProcPtr = FUNCTION(inMenu: MenuHandle; inRequest: HMContentRequest; VAR outContentProvided: HMContentProvidedType; ioHelpContent: HMHelpContentPtr): OSStatus;
{$ELSEC}
...
FUNCTION InvokeHMWindowContentUPP(inWindow: WindowPtr; inGlobalMouse: Point; inRequest: HMContentRequest; VAR outContentProvided: HMContentProvidedType; ioHelpContent: HMHelpContentPtr; userRoutine: HMWindowContentUPP): OSStatus; { old name was CallHMWindowContentProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}

FUNCTION InvokeHMMenuTitleContentUPP(inMenu: MenuHandle; inRequest: HMContentRequest; VAR outContentProvided: HMContentProvidedType; ioHelpContent: HMHelpContentPtr; userRoutine: HMMenuTitleContentUPP): OSStatus; { old name was CallHMMenuTitleContentProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}

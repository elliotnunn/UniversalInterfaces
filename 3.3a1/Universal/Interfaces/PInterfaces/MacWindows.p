...
CONST
	...
	kDesktopWindowClass			= 7;							{  desktop window (usually only one of these exists) - OS X only in CarbonLib 1.0 }
...
{——————————————————————————————————————————————————————————————————————————————————————}
	...
	gestaltWindowMgrPresent		= $00000001;					{  NOTE: this is a bit mask, where all other Gestalt constants of }
...
FUNCTION GetFrontWindowOfClass(inWindowClass: WindowClass; mustBeVisible: BOOLEAN): WindowPtr;
FUNCTION FindWindowOfClass({CONST}VAR where: Point; inWindowClass: WindowClass; VAR outWindow: WindowRef; VAR outWindowPart: SInt16): OSStatus;
...
FUNCTION IsValidWindowPtr(possibleWindow: WindowPtr): BOOLEAN;
{  Routines available from Mac OS 8.6 forward }
{
...
FUNCTION WindowPathSelect(window: WindowRef; menu: MenuHandle; VAR outMenuResult: SInt32): OSStatus;
...
FUNCTION ZoomWindowIdeal(window: WindowRef; partCode: SInt16; VAR ioIdealSize: Point): OSStatus;
...
{$IFC TARGET_CARBON }
{$IFC CALL_NOT_IN_CARBON }
PROCEDURE DebugPrintWindow(windowRef: WindowRef);
PROCEDURE DebugPrintWindowList;
PROCEDURE DebugInvertVisRegion(windowRef: WindowRef);
{$ENDC}  {CALL_NOT_IN_CARBON}
{$ENDC}  {TARGET_CARBON}

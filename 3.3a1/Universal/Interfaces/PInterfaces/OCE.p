...
FUNCTION OCERToPString(rStr: {Const}RStringPtr): StringPtr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION OCEUnpackPathName({CONST}VAR path: PackedPathName; VAR parts: RString; nParts: UInt16): UInt16;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION OCEPackPathName({CONST}VAR parts: RStringPtr; nParts: UInt16; VAR path: PackedPathName; pathLength: UInt16): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION OCEPackedRLIPartsSize({CONST}VAR dirName: DirectoryName; {CONST}VAR parts: RStringPtr; nParts: UInt16): UInt16;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION OCEPackRLIParts({CONST}VAR dirName: DirectoryName; {CONST}VAR discriminator: DirDiscriminator; dNodeNumber: DNodeNum; {CONST}VAR parts: RStringPtr; nParts: UInt16; VAR prli: PackedRLI; prliLength: UInt16): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION NewDSSpecStreamerUPP(userRoutine: DSSpecStreamerProcPtr): DSSpecStreamerUPP; { old name was NewDSSpecStreamerProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}

PROCEDURE DisposeDSSpecStreamerUPP(userUPP: DSSpecStreamerUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}

FUNCTION InvokeDSSpecStreamerUPP(buffer: UNIV Ptr; count: UInt32; eof: BOOLEAN; userData: LONGINT; userRoutine: DSSpecStreamerUPP): OSErr; { old name was CallDSSpecStreamerProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}
{$ENDC}  {CALL_NOT_IN_CARBON}
...
FUNCTION NewOCERecipientStreamerUPP(userRoutine: OCERecipientStreamerProcPtr): OCERecipientStreamerUPP; { old name was NewOCERecipientStreamerProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}

PROCEDURE DisposeOCERecipientStreamerUPP(userUPP: OCERecipientStreamerUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}

FUNCTION InvokeOCERecipientStreamerUPP(buffer: UNIV Ptr; count: UInt32; eof: BOOLEAN; userData: LONGINT; userRoutine: OCERecipientStreamerUPP): OSErr; { old name was CallOCERecipientStreamerProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}
{$ENDC}  {CALL_NOT_IN_CARBON}

...
FUNCTION NewIPMNoteUPP(userRoutine: IPMNoteProcPtr): IPMNoteUPP; { old name was NewIPMNoteProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}

PROCEDURE DisposeIPMNoteUPP(userUPP: IPMNoteUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}

PROCEDURE InvokeIPMNoteUPP(queue: IPMQueueRef; seqNum: IPMSeqNum; notificationType: ByteParameter; userData: UInt32; userRoutine: IPMNoteUPP); { old name was CallIPMNoteProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}
...
FUNCTION NewIPMIOCompletionUPP(userRoutine: IPMIOCompletionProcPtr): IPMIOCompletionUPP; { old name was NewIPMIOCompletionProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}

PROCEDURE DisposeIPMIOCompletionUPP(userUPP: IPMIOCompletionUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}

PROCEDURE InvokeIPMIOCompletionUPP(paramBlock: IPMParamBlockPtr; userRoutine: IPMIOCompletionUPP); { old name was CallIPMIOCompletionProc }
{$ENDC}  {CALL_NOT_IN_CARBON}

{
 	...
 	Copyright:	© 1994-1999, 1999 by Apple Computer, Inc., all rights reserved.
...
FUNCTION NewSMPDrawImageUPP(userRoutine: SMPDrawImageProcPtr): SMPDrawImageUPP; { old name was NewSMPDrawImageProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}

PROCEDURE DisposeSMPDrawImageUPP(userUPP: SMPDrawImageUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}

PROCEDURE InvokeSMPDrawImageUPP(refcon: LONGINT; inColor: BOOLEAN; userRoutine: SMPDrawImageUPP); { old name was CallSMPDrawImageProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}
...
FUNCTION NewFrontWindowUPP(userRoutine: FrontWindowProcPtr): FrontWindowUPP; { old name was NewFrontWindowProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}

FUNCTION NewPrepareMailerForDrawingUPP(userRoutine: PrepareMailerForDrawingProcPtr): PrepareMailerForDrawingUPP; { old name was NewPrepareMailerForDrawingProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}

PROCEDURE DisposeFrontWindowUPP(userUPP: FrontWindowUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}

PROCEDURE DisposePrepareMailerForDrawingUPP(userUPP: PrepareMailerForDrawingUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}

FUNCTION InvokeFrontWindowUPP(clientData: LONGINT; userRoutine: FrontWindowUPP): WindowPtr; { old name was CallFrontWindowProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}

PROCEDURE InvokePrepareMailerForDrawingUPP(window: WindowPtr; clientData: LONGINT; userRoutine: PrepareMailerForDrawingUPP); { old name was CallPrepareMailerForDrawingProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}
{$ENDC}  {CALL_NOT_IN_CARBON}
...
FUNCTION NewSendOptionsFilterUPP(userRoutine: SendOptionsFilterProcPtr): SendOptionsFilterUPP; { old name was NewSendOptionsFilterProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}

PROCEDURE DisposeSendOptionsFilterUPP(userUPP: SendOptionsFilterUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}

FUNCTION InvokeSendOptionsFilterUPP(theDialog: DialogPtr; VAR theEvent: EventRecord; itemHit: INTEGER; clientData: LONGINT; userRoutine: SendOptionsFilterUPP): BOOLEAN; { old name was CallSendOptionsFilterProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $205F, $4E90;
	{$ENDC}
{$ENDC}  {CALL_NOT_IN_CARBON}
...
FUNCTION SMPGetMainEnclosureFSSpec(window: WindowPtr; VAR enclosureDir: FSSpec): OSErr;
	...
	{$ENDC}
FUNCTION SMPAddContent(window: WindowPtr; segmentType: MailSegmentType; appendFlag: BOOLEAN; buffer: UNIV Ptr; bufferSize: UInt32; textScrap: StScrpRecPtr; startNewScript: BOOLEAN; script: ScriptCode): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $203C, $000C, $127A, $AA5D;
	{$ENDC}
FUNCTION SMPReadContent(window: WindowPtr; segmentTypeMask: MailSegmentMask; buffer: UNIV Ptr; bufferSize: UInt32; VAR dataSize: UInt32; textScrap: StScrpRecPtr; VAR script: ScriptCode; VAR segmentType: MailSegmentType; VAR endOfScript: BOOLEAN; VAR endOfSegment: BOOLEAN; VAR endOfContent: BOOLEAN; VAR segmentLength: LONGINT; VAR segmentID: LONGINT): OSErr;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}

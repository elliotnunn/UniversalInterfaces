...
TYPE
	FNSubscriptionRef    = ^LONGINT; { an opaque 32-bit type }
...
TYPE
{$IFC TYPED_FUNCTION_POINTERS}
	FNSubscriptionProcPtr = PROCEDURE(message: FNMessage; flags: OptionBits; refcon: UNIV Ptr; subscription: FNSubscriptionRef); C;
{$ELSEC}
	FNSubscriptionProcPtr = ProcPtr;
{$ENDC}

{$IFC OPAQUE_UPP_TYPES}
	FNSubscriptionUPP = ^LONGINT; { an opaque UPP }
{$ELSEC}
...
FUNCTION NewFNSubscriptionUPP(userRoutine: FNSubscriptionProcPtr): FNSubscriptionUPP;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}
...
PROCEDURE DisposeFNSubscriptionUPP(userUPP: FNSubscriptionUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}
...
PROCEDURE InvokeFNSubscriptionUPP(message: FNMessage; flags: OptionBits; refcon: UNIV Ptr; subscription: FNSubscriptionRef; userRoutine: FNSubscriptionUPP);
{
 *  FNSubscribe()
 *  
 *  Summary:
...
FUNCTION FNSubscribe({CONST}VAR directoryRef: FSRef; callback: FNSubscriptionUPP; refcon: UNIV Ptr; flags: OptionBits; VAR subscription: FNSubscriptionRef): OSStatus; C;
...
FUNCTION FNSubscribeByPath({CONST}VAR directoryPath: UInt8; callback: FNSubscriptionUPP; refcon: UNIV Ptr; flags: OptionBits; VAR subscription: FNSubscriptionRef): OSStatus; C;
...
FUNCTION FNUnsubscribe(subscription: FNSubscriptionRef): OSStatus; C;
...
FUNCTION FNGetDirectoryForSubscription(subscription: FNSubscriptionRef; VAR ref: FSRef): OSStatus; C;

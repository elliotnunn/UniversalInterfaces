...
CONST
	...
	internal_layout_error		= -27948;

																{  recoverable errors  }
	could_not_dispose_backing_store = -27946;
	unflattening_interrupted_by_client = -27945;

																{  font manager errors  }
	font_cannot_be_changed		= -27944;
	illegal_font_parameter		= -27943;
	...
	unknown_font_scaler_error	= -27880;
	...
	invalid_viewPort_reference	= -27835;
	...
	cannot_dispose_locked_shape	= -27658;
	...
	layer_glyph_shape_cannot_contain_nil_styles = -27617;
	...
	bitmap_rowBytes_must_be_specified_for_user_image_buffer = -27567;
	...
	metrics_not_in_metrics_list	= -27522;
	...
	invalid_bitmap_in_device	= -27516;
	...
	profile_response_out_of_range = -26889;
	...
	shape_direct_attribute_not_set = -26687;
	...
	insufficient_coordinate_space_for_new_device = -26682;
...
{$IFC TYPED_FUNCTION_POINTERS}
	gxUserErrorProcPtr = PROCEDURE(status: gxGraphicsError; refcon: LONGINT); C;
{$ELSEC}
	gxUserErrorProcPtr = ProcPtr;
{$ENDC}

{$IFC TYPED_FUNCTION_POINTERS}
	gxUserWarningProcPtr = PROCEDURE(status: gxGraphicsWarning; refcon: LONGINT); C;
{$ELSEC}
	gxUserWarningProcPtr = ProcPtr;
{$ENDC}

{$IFC TYPED_FUNCTION_POINTERS}
	gxUserNoticeProcPtr = PROCEDURE(status: gxGraphicsNotice; refcon: LONGINT); C;
{$ELSEC}
	gxUserNoticeProcPtr = ProcPtr;
{$ENDC}

{$IFC OPAQUE_UPP_TYPES}
	gxUserErrorUPP = ^LONGINT; { an opaque UPP }
{$ELSEC}
	gxUserErrorUPP = UniversalProcPtr;
{$ENDC}	
{$IFC OPAQUE_UPP_TYPES}
	gxUserWarningUPP = ^LONGINT; { an opaque UPP }
{$ELSEC}
	gxUserWarningUPP = UniversalProcPtr;
{$ENDC}	
{$IFC OPAQUE_UPP_TYPES}
	gxUserNoticeUPP = ^LONGINT; { an opaque UPP }
{$ELSEC}
...
FUNCTION NewgxUserErrorUPP(userRoutine: gxUserErrorProcPtr): gxUserErrorUPP; { old name was NewgxUserErrorProc }
...
FUNCTION NewgxUserWarningUPP(userRoutine: gxUserWarningProcPtr): gxUserWarningUPP; { old name was NewgxUserWarningProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}
...
FUNCTION NewgxUserNoticeUPP(userRoutine: gxUserNoticeProcPtr): gxUserNoticeUPP; { old name was NewgxUserNoticeProc }
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}
...
PROCEDURE DisposegxUserErrorUPP(userUPP: gxUserErrorUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}
...
PROCEDURE DisposegxUserWarningUPP(userUPP: gxUserWarningUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}
...
PROCEDURE DisposegxUserNoticeUPP(userUPP: gxUserNoticeUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}
...
PROCEDURE InvokegxUserErrorUPP(status: gxGraphicsError; refcon: LONGINT; userRoutine: gxUserErrorUPP); { old name was CallgxUserErrorProc }
{
...
PROCEDURE InvokegxUserWarningUPP(status: gxGraphicsWarning; refcon: LONGINT; userRoutine: gxUserWarningUPP); { old name was CallgxUserWarningProc }
{
...
PROCEDURE InvokegxUserNoticeUPP(status: gxGraphicsNotice; refcon: LONGINT; userRoutine: gxUserNoticeUPP); { old name was CallgxUserNoticeProc }

...
{$IFC TYPED_FUNCTION_POINTERS}
	IDHNotificationProcPtr = FUNCTION(VAR event: IDHGenericEvent; userData: UNIV Ptr): OSStatus; C;
{$ELSEC}
	IDHNotificationProcPtr = ProcPtr;
{$ENDC}

	IDHNotificationProc					= IDHNotificationProcPtr;
{$IFC OPAQUE_UPP_TYPES}
	IDHNotificationUPP = ^LONGINT; { an opaque UPP }
{$ELSEC}
	IDHNotificationUPP = IDHNotificationProcPtr;
{$ENDC}	
	...
	IDHParameterBlock = RECORD
		...
		completionProc:			IDHNotificationUPP;
		refCon:					Ptr;
		result:					OSErr;
	END;
...
FUNCTION IDHNewNotification(idh: ComponentInstance; deviceID: IDHDeviceID; notificationProc: IDHNotificationUPP; userData: UNIV Ptr; VAR notificationID: IDHNotificationID): ComponentResult;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
FUNCTION IDHGetDeviceTime(idh: ComponentInstance; VAR deviceTime: TimeRecord): ComponentResult;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
...
CONST
	uppIDHNotificationProcInfo = $000003F1;
	{
...
FUNCTION NewIDHNotificationUPP(userRoutine: IDHNotificationProcPtr): IDHNotificationUPP;
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $2E9F;
	{$ENDC}
...
PROCEDURE DisposeIDHNotificationUPP(userUPP: IDHNotificationUPP);
	{$IFC TARGET_OS_MAC AND TARGET_CPU_68K AND NOT TARGET_RT_MAC_CFM}
	INLINE $584F;
	{$ENDC}
...
FUNCTION InvokeIDHNotificationUPP(VAR event: IDHGenericEvent; userData: UNIV Ptr; userRoutine: IDHNotificationUPP): OSStatus;

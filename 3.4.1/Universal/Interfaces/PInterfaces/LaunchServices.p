...
CONST
	...
	kLSMinCatInfoBitmap			= $0000180A;					{  minimum info needed to avoid a FSGetCatalogInfo call when fetching item information  }
...
CONST
	...
	kLSRequestBasicFlagsOnly	= $00000004;					{  all but type of application and extension flags - safe to use from threads }
	...
	kLSRequestExtensionFlagsOnly = $00000040;					{  safe to use from threads }
...
TYPE
	...
	LSItemInfoRecord = RECORD
		...
		filetype:				OSType;
		creator:				OSType;									{  release when finished }
...
FUNCTION LSGetExtensionInfo(inNameLen: UniCharCount; {CONST}VAR inNameBuffer: UniChar; VAR outExtStartIndex: UniCharCount): OSStatus;
...
FUNCTION LSCopyDisplayNameForRef({CONST}VAR inRef: FSRef; VAR outDisplayName: CFStringRef): OSStatus;
...
FUNCTION LSCopyDisplayNameForURL(inURL: CFURLRef; VAR outDisplayName: CFStringRef): OSStatus;
...
FUNCTION LSSetExtensionHiddenForRef({CONST}VAR inRef: FSRef; inHide: BOOLEAN): OSStatus;
...
FUNCTION LSSetExtensionHiddenForURL(inURL: CFURLRef; inHide: BOOLEAN): OSStatus;

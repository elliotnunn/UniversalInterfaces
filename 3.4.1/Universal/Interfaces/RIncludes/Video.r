...
#define kModeNotGraphicsQuality 		12					/*  Indicates that the display mode is not the highest quality (eg. stretching artifacts).  Intended as a hint  */
...
#define kHardwarePark 					133
#define kHardwareDrive 					134
...
#define cscSetScaler 					36					/*  Takes a VDScalerPtr */
#define cscSetMirror 					37					/*  Takes a VDMirrorPtr */
...
#define cscGetScalerInfo 				35					/*  Takes a VDScalerInfoPtr  */
#define cscGetScaler 					36					/*  Takes a VDScalerPtr */
#define cscGetMirror 					37					/*  Takes a VDMirrorPtr */
...
#define kVideoCannotMirrorErr 			(-10936)
...
#define kSyncPositivePolarityMask 		0x01
															...
															/*  csScalerFeatures */
#define kScaleStretchOnlyMask 			0x01				/*  True means the driver cannot add borders to avoid non-square pixels */
#define kScaleCanUpSamplePixelsMask 	0x02				/*  True means timings with more active clocks than pixels (ie 640x480 pixels on a 1600x1200 timing) */
#define kScaleCanDownSamplePixelsMask 	0x04				/*  True means timings with fewer active clocks than pixels (ie 1600x1200  pixels on a 640x480 timing) */

															/*  csScalerFlags */
#define kScaleStretchToFitMask 			0x01				/*  True means the driver should avoid borders and allow non-square pixels */
...
#define kSetClutAtVBL 					1					/*  SetEntries behavior is to upate clut at next vbl */
															...
															/*  csMirrorFeatures */
#define kMirrorSameDepthOnlyMirrorMask 	0x01				/*  Commonly true - Mirroring can only be done if the displays are the same bitdepth */
#define kMirrorSameSizeOnlyMirrorMask 	0x02				/*  Commonly false - Mirroring can only be done if the displays are the same size */
#define kMirrorSameTimingOnlyMirrorMask  0x04				/*  Sometimes true - Mirroring can only be done if the displays are the same timing */
#define kMirrorCommonGammaMask 			0x08				/*  Sometimes true - Only one gamma correction LUT. */

															/*  csMirrorSupportedFlags and csMirrorFlags */
#define kMirrorCanMirrorMask 			0x01				/*  Set means we can HW mirrored right now (uses csMirrorEntryID) */
#define kMirrorAreMirroredMask 			0x02				/*  Set means we are HW mirrored right now (uses csMirrorEntryID) */
#define kMirrorUnclippedMirrorMask 		0x04				/*  Set means mirrored displays are not clipped to their intersection */
#define kMirrorHAlignCenterMirrorMask 	0x08				/*  Set means mirrored displays can/should be centered horizontally */
#define kMirrorVAlignCenterMirrorMask 	0x10				/*  Set means mirrored displays can/should be centered vertically */
#define kMirrorCanChangePixelFormatMask  0x20				/*  Set means mirrored the device should change the pixel format of mirrored displays to allow mirroring. */
#define kMirrorCanChangeTimingMask 		0x40				/*  Set means mirrored the device should change the timing of mirrored displays to allow mirroring. */
